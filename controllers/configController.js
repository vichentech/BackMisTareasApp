...
class ConfigController {
  ...
  async updateMasterLists(req, res, next) {
    try {
      ...
      const { projects, mainTasks, vehicles, otherWorkTypes } = req.body;
      ...
      if (!Array.isArray(projects) || !Array.isArray(mainTasks) || !Array.isArray(vehicles) || !Array.isArray(otherWorkTypes)) {
        return res.status(400).json({
          success: false,
          message: 'Todos los campos deben ser arrays'
        });
      }

      console.log(`[CONFIG] Reemplazando listas: ${projects.length} proyectos, ${mainTasks.length} tareas, ${vehicles.length} vehículos, ${otherWorkTypes.length} otros tipos`);

      const result = await configService.updateMasterLists(projects, mainTasks, vehicles, otherWorkTypes);
