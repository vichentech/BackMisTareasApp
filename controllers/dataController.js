...
class DataController {
  ...
  async syncCheckPost(req, res, next) {
    try {
      ...
      const { localTimestamps, year, supportsHashing } = req.body;
      const { db, collection } = req.query;
      ...
      if (supportsHashing) {
        console.log(`[SYNC_CHECK_POST] Cliente soporta hashing`);
      }
      ...
      if (
        ...
        return res.status(400).json({
          success: false,
          message:
            "El parámetro year debe ser un número válido entre 1900 y 2100",
        });
      }
      ...
      if (result.success) {
        console.log(
          `[SYNC_CHECK_POST] Para subir: ${
            result.daysToUpload.length
          }, para descargar: ${result.daysToDownload.length}, conflictos: ${
            result.lockedConflicts.length
          }, timestamps a actualizar: ${
            result.timestampsToUpdate ? result.timestampsToUpdate.length : 0
          }`
        );
      }
      console.log("");
      ...
      return res.status(result.statusCode || 200).json({
        ...
        timestampsToUpdate: result.timestampsToUpdate || [],
        message: result.message,
      });
    } catch (error) {
